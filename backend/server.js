...
app.post('/api/orders',(req,res)=>{ const o={_id:Date.now().toString(),id:Date.now(),...req.body,date:new Date()}; memOrders.push(o); console.log('ORDER',o.customerName); res.json({ok:true}); });
app.delete('/api/orders/:id',(req,res)=>{ memOrders=memOrders.filter(x=>x._id!=req.params.id && x.id!=req.params.id); res.json({ok:true}); });
app.get('/api/health',(req,res)=>res.json({ok:true,count:memProducts.length}));

const fs=require('fs');
const fp=path.join(__dirname,'../frontend');
if(fs.existsSync(fp)) app.use(express.static(fp));

app.use((req,res)=>{
  const idx=path.join(__dirname,'../frontend/index.html');
  if(fs.existsSync(idx)) return res.sendFile(idx);
  res.json({ok:true, msg:'API running', products:'/api/products', health:'/api/health'});
});
