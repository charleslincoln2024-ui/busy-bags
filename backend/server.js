...
console.log('MONGO exists?', !!MONGO);
if(MONGO) mongoose.connect(MONGO).then(()=>console.log('Mongo connected')).catch(e=>console.log('Mongo error',e.message));
...
let Order; try{ Order=require('./models/Order'); }catch(e){ Order=mongoose.model('Order', new mongoose.Schema({customerName:String,name:String,phone:String,address:String,items:Array,products:Array,total:Number,totalAmount:Number,date:Date},{strict:false,timestamps:true})); }

async function seed(){
 try{
  if(!mongoose.connection.readyState) return setTimeout(seed,3000);
  ...
  if(count===0){
    ...
    console.log('SEEDED '+prods.length);
  }
 }catch(e){console.log('seed error',e.message);}
}
setTimeout(seed,2000);

app.use('/api/auth', (req,res,next)=>{ try{ return require('./routes/auth')(req,res,next); }catch(e){ try{ return require('./routes/auth').default||require('./routes/auth'); }catch(e2){ next(); } } });
app.use('/api/auth', require('./routes/auth'));

let prodRouter; try{ prodRouter=require('./routes/products'); app.use('/api/products',prodRouter); }catch(e){ console.log('use custom products'); app.get('/api/products',async(req,res)=>{try{const p=await Product.find();res.json(p);}catch(e){res.json([]);}}); app.post('/api/products',async(req,res)=>{try{const p=await Product.create(req.body);res.json(p);}catch(e){res.status(500).json({error:e.message});}}); app.delete('/api/products/:id',async(req,res)=>{try{await Product.findByIdAndDelete(req.params.id);res.json({ok:true});}catch(e){res.status(500).json({error:e.message});}}); }

let orderRouter; try{ orderRouter=require('./routes/orders'); app.use('/api/orders',orderRouter); }catch(e){ console.log('use custom orders'); app.get('/api/orders',async(req,res)=>{try{const o=await Order.find().sort({createdAt:-1});res.json(o);}catch(e){res.json([]);}}); app.post('/api/orders',async(req,res)=>{try{const o=await Order.create({...req.body,date:new Date()});res.json({ok:true,id:o._id});}catch(e){res.status(500).json({error:e.message});}}); app.delete('/api/orders/:id',async(req,res)=>{try{await Order.findByIdAndDelete(req.params.id);res.json({ok:true});}catch(e){res.status(500).json({error:e.message});}}); }

app.get('*',(req,res)=>{ const idx=path.join(__dirname,'../frontend/index.html'); const fs=require('fs'); if(fs.existsSync(idx)) return res.sendFile(idx); res.send('API running <a href="/api/products">/api/products</a>'); });
const PORT=process.env.PORT||10000; app.listen(PORT,()=>console.log('READY '+PORT));
